#!/usr/bin/env python3
"""
Railway production bot - с исправленной командой /profile
"""
    ...
    try:
        ...
        async def help_handler(message: Message):
            await message.answer(
                ...
                "/clear - Очистить все данные (для тестирования)\n\n"
        ...
        async def profile_handler(message: Message):
            ...
            try:
                ...
                photo_info = []
                photo_types = {"selfie": "Селфи", "full_body": "Полный рост", "clothing": "Одежда"}
                
                for photo in photos:
                    photo_type_name = photo_types.get(photo[0], photo[0])
                    photo_info.append(f"• {photo_type_name}: ✅")
        ...
        async def clear_handler(message: Message, state: FSMContext):
            ...
            success = clear_user_data(message.from_user.id)
            
            if success:
                await message.answer(
                    "🧹 <b>Данные очищены!</b>\n\n"
                    "✅ Все ваши фото удалены\n"
                    "✅ Профиль сброшен\n"
                    "✅ Состояние очищено\n\n"
